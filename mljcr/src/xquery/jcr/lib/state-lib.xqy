...
(: Find a node's name by looking it up by in in the change list.
...
(: Take a binary document, which was inserted in a temp location along with
   the changelist, and move it to its final location.  The document is also
   inserted with the appropriate format for its content.  We do not know what
   the original name of the document was, so there are no hints as to type.
   We try as XML, text and binary, in that order, and depend on an exception
   to tell us if it succeeded or not.  Any collections defined for the workspace
   are also applied.
 :)
...
(: Return a new property, which is the input property element unless
   it's of type binary.  For binary properties, an associated document
   may be created.
 :)
...
(: If the candidate node does not exist in the sequence of existing nodes,
   ...
   node lives in another workspace, which is allowed by JSR-170.
 :)
...
(: Apply updates for this node.  An update could mean creating a new node,
   ...
   Note that a lot of function mapping is foing on here.
 :)
...
(: Apply updates to the workspace tree (adds and modifies).  This
   ...
   pruned away.
   To improve performance, several maps are used to avoid repeated
   traversals in-memory trees.  That can cause cache thrashing when
   the trees get big.
 :)
...
(: Attempt to prune this node.  Any child nodes and/or properties are
   first pruned in case they have been deleted (this is needed because
   child nodes may have associated external documents, which need to be
   deleted).  Then the change list is checked to see if this node has
   been deleted.  If so, return the empty sequence.  If not deleted,
   return a copy of this node with the surviving child nodes.
...
(: Returns a list of URI information for newly created external documents
   (binary properties).  This info is passed back to Java so that it can
   update its model to match what's been persisted.
 :)
